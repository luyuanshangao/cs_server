...
class UniQueue extends Controller
{
    /** @var Assets */
    private $assets;
    /** @var User */
    ...
    public function listener()
    {
        ...
        for ($i = 0; $i < count($userList); $i++) {
            ...
            //查询用户eth钱包金额变动
            $amount = $uniswap->getBalanceOfAddress($userInfo["ethAddress"]);
            if ($amount["balance"] > 0) {
                //准备将金额转出
                $re = $uniswap->sendUNI($userInfo["ethAddress"], $this->baseEthAddress, $amount["balance"]);
                if ($re) {
                    //将金额存入账户
                    $this->assets->addUNI($userInfo["userId"], $amount["balance"], "UNI充值");
                }
            }
        }
    }
